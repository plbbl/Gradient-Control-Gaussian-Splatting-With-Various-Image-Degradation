...
def SE3_to_se3(Rt, eps=1e-8):  # [...,3,4]
    R = Rt[..., :3, :3]  # 提取前3行前3列，即旋转矩阵
    t = Rt[..., :3, 3:]   # 提取前3行最后1列，即平移向量
...
def gard_Tt_Tstart_se3(start_pose, end_pose, idx,poses_number, NUM, device=None):
    ...
    q_t_1 = exp_r2q_parallel(-r)
    #q_t = q_to_Q_parallel(q_start) @ q_t_0[..., None]

    # 计算伴随映射
    R = q_to_R_parallel(q_t_1.squeeze(dim=-1))
    R = R[0,0]
    
    AD=SO3_to_so3(R)
    
    
    Tt_Tstart_se3=R*AD*(idx/NUM)
    #w = skew_symmetric(r)
    #Ad = torch.eye(3) + (1 - torch.cos(w.norm())) / w.norm()**2 * w + (w.norm() - torch.sin(w.norm())) / w.norm()**3 * w @ w

    return Tt_Tstart_se3

def gard_Tt_Tend_se3(start_pose, end_pose, idx,poses_number, NUM, device=None):
    pose_time = poses_number / (NUM - 1)
